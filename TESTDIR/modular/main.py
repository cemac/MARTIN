...
from tkinter import colorchooser, filedialog, ttk, StringVar
import PIL.Image
import PIL.ImageTk
...
class Main(ttk.Frame):
    def __init__(self, master, source_dir='.', run_after_selection=None):
        super().__init__()
        self.fore_dict = {}
        # define lists that will be used to populate dictionaries based on
        # directory structure
        self.master.source_dir = StringVar(self)
        self.master.source_dir.set(source_dir)
        source_list = next(os.walk(self.master.source_dir.get()))[1]
        region_list = []
        init_list = []
        rr_list = []
        region_count = 0
        ...
        self.master.source_dir.trace('w', self.update_path)
        self.optionmenu0 = tk.OptionMenu(self, self.master.source_dir.get(), *self.fore_dict.keys())
        tk.Label(self, text="Source").grid(row=1, column=3, columnspan=3)
        self.optionmenu0.grid(row=2, column=1, columnspan=3)
    ...
    def update_path(self, *args):
        source_list_new = []
        source_list_new = next(os.walk(self.master.source_dir.get()))[1]
        self.fore_dict = {}
        region_list = []
        init_list = []
        rr_list = []
        ...
        var_count = 0
        # loop through top directory
        for i in range(0, len(source_list_new)):
            # check and populate first level forecast dictionary
            if not source_list_new[i] in self.fore_dict:
                self.fore_dict[source_list_new[i]] = {}
            # define values in second level of directory structure and add to
            # list for use in dictionary
            region_list.append(next(os.walk(source_list_new[i] + "/."))[1])
            # loop through second level of directories
            for j in range(0, len(region_list[region_count])):
                # check and populate second level forecast dictionary
                if not region_list[region_count][j] in self.fore_dict[source_list_new[i]]:
                    self.fore_dict[source_list_new[i]
                                   ][region_list[region_count][j]] = {}
                # define values in third level of directory structure and add
                # to list for use in dictionary
                init_list.append(next(os.walk(source_list_new[i] + "/" +
                                      region_list[region_count][j] + "/."))[1])
                # loop through third level of directories
                for k in range(0, len(init_list[init_count])):
                    # check and populate third level forecast dictionary
                    if not init_list[init_count][k] in self.fore_dict[source_list[i]][region_list[region_count][j]]:
                        self.fore_dict[source_list_new[i]][region_list[region_count]
                                                       [j]][init_list[init_count][k]] = {}
                    # define values in fourth level of directory structure and
                    # add to list for use in dictionary
                    var_list.append(next(os.walk(source_list_new[i] + "/" +
                                                 region_list[region_count][j]
                                                 + "/" +
                                                 init_list[init_count][k] +
                                                 "/."))[1])
                    ...
                    for l in range(0, len(var_list[var_count])):
                        temp_file_names = glob.glob(
                            source_list_new[i] + "/" + region_list[region_count][j] + "/" + init_list[init_count][k] + "/" + var_list[var_count][l] + "/*")
                        temp_fores_list = []
                        for tfn in temp_file_names:
                            tfn = os.path.basename(tfn)
                            if "analysis" in tfn:
                        ...
                        if not var_list[var_count][l] in self.fore_dict[source_list_new[i]][region_list[region_count][j]][init_list[init_count][k]]:
                            self.fore_dict[source_list[i]][region_list[region_count][j]][init_list[init_count]
                                                                                         [k]][var_list[var_count][l]] = list(sorted(set(temp_fores_list)))
                ...
                init_count = init_count + 1
            region_count = region_count + 1
        sources = self.fore_dict[source_list_new]
        menu = self.optionmenu1['menu']
        menu.delete(0, 'end')
        for source in sources:
            menu.add_command(
                label=source, command=lambda sou=source: self.var1.set(sou))
